...
public class PostingsList implements QueryResult {
	...
	/**
	 * add a document ID to this posting list, with a given weight
	 * 
	 * @param docID
	 *            the docID of the document being added
	 * @param weight
	 *            the weight of the document being added
	 */
	public void addDoc(int docID, double weight) {
		if (head == null) {
			head = new Node(docID, weight);
			tail = head;
		} else {
			tail.setNext(new Node(docID, weight));
			tail = tail.next();
		}
	...
	/**
	 * multiply all of the weights by
	 * log(number_of_docs/number_of_docs_with_this_term)
	 */
	...
	/**
	 * add one to the tf count for the last entry in the posting list as a
	 * sanity check, we'll require the previous docID to be passed in and check
	 * this against the last element in our posting list
	 * 
	 * @param docID
	 */
	public void incrementTFOfLastEntry(int docID) {
		if (docID != tail.docID()) {
			throw new RuntimeException(
					"PostingsList::addOneToLastEntry - Tried to add to wrong docID "
							+ docID + " vs. " + tail.docID());
		}
	 ...
	 * 
	 * The version I gave you decouplee the Index class and the PostingsList
	 * class but I didn't have time to update my solution :)
	 * 
	 * @param result
	 * @param termWeight
	 */
	...
	/**
	 * Given a postings list, return the NOT of the postings list, i.e. a
	 * postings list that contains all document ids not in "list"
	 * 
	...
	public static PostingsList not(PostingsList list, int maxDocID) {
		...
		// add all the remaining entries from the last doc in the posting list
		// to the
		// last available doc
	...
	/**
	 * Given two postings lists, return a new postings list that contains the
	 * AND of the postings, i.e. all the docIDs that occur in both posting1 and
	 * posting2
	 * 
	 * @param posting1
	 * @param posting2
	 ...
	 */
	public static PostingsList andMerge(PostingsList posting1,
	 ...
	 * of the postings, i.e. all those docIDs that occur in either posting1 and
	 * posting2
	 * 
	 * @param posting1
	 * @param posting2
	 ...
	 */
	public static PostingsList orMerge(PostingsList posting1,
			PostingsList posting2) {
		PostingsList merged = new PostingsList();
	...
	/**
	 * From the linked list structure, generate an integer array containing all
	 * of the document ids. This will make our life easy when we want to print
	 * out the ids. (another option would have been to write an iterator, but
	 * this is easier).
	 * 
	 * @return
	 */
	...
	/**
	 * A private node class for creating a linked list within the PostingsList
	 * class
	 * 
	 * @author dkauchak
	 * 
